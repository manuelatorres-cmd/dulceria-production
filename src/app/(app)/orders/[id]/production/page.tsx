...
export default function OrderProductionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="px-4 py-6 max-w-2xl mx-auto">
      ...
      {linkedPlans.length === 0 ? (
        <div className="rounded-[14px] border border-dashed border-border bg-card/60 p-8 text-center">
          ...
          {planMeta.map(({ plan, productNames, totalQty, earliestDate, doneCount, status }) => (
            <li key={plan.id}>
              <Link
                ...
                className="flex items-center gap-3 rounded-[14px] border border-border bg-card/80 px-4 py-3 hover:border-foreground/30 transition"
              >
